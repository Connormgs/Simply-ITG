...
function JudgmentTween(self) self:diffusealpha(1) self:zoomx(1.3) self:zoomy(1.7) self:decelerate(0.1) self:zoom(1) self:sleep(1) self:accelerate(0.2) self:zoom(0) self:glowshift() self:effectperiod(0.05) self:effectcolor1(1,1,1,0) self:effectcolor2(1,1,1,0.5) end
...
-- Judgment Font List
judgmentFontList = { 'Bebas2', 'Chromatic', 'Code2', 'Comic Sans2', 'Deco', 'DDR A', 'DDR Extreme', 'ECFA', 'ECFA No Quotes', 'Emoji', 'FP', 'GrooveNights', 'ITG2', 'Japanese', 'Lobster2', 'Love', 'Love Chroma', 'Miso2', 'mute', 'Optimus Dark', 'Rainbowmatic', 'Roboto', 'Roboto ITG', 'Tactics', 'VHS', 'Vision', 'Vision Dark', 'Wendy2', 'IIDX', }
...
for pn=1,2 do if Player(pn) then
	...
	if (MaxDP[pn] or -1) > 0 then
		if (GetScore(pn) <= 100*judge[pn].CurDP/MaxDP[pn] - 0.01) and GetScore(pn) ~= judge[pn].Score then TrackJudgment(false,9,pn) end -- Score must have changed to reflect judgment, machines update slower.
		UpdateGhostData(pn)
		UpdateStreams(pn)
		UpdateCompareText(pn)
		UpdateMeasureText(pn)
	end
	if Dead.State[pn] and not judge[pn].Dead then judge[pn].Dead = table.getn(judge[pn].Data) end -- Record judgment of death, checked after mine detection.
end end
end
...
if not GAMESTATE:GetCurrentSong() then
	for pn=1,2 do if Player(pn) then listPointerY[pn] = Difficulty[pn] + 1 end end
	return
end
